...
void RakClient::Disconnect( unsigned int blockDuration, unsigned char orderingChannel, bool forceExit)
{
	RakPeer::Disconnect( blockDuration, orderingChannel, forceExit );
}
...
Packet* RakClient::Receive( void )
{
	...
	if ( packet )
	{
		...
		DbgOut("RakClient::Receive packet id %i", packet->data[0]);
...
int RakClient::GetLastRPCID(void)
{
	return RakPeer::GetLastRPCID();
}

int RakClient::GetLastPacketID(void)
{
	return RakPeer::GetLastPacketID();
}
