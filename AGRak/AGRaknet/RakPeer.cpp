...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::Disconnect( unsigned int blockDuration, unsigned char orderingChannel, bool forceExit)
{
	unsigned i,j;
	bool anyActive;
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
		...
		DbgOut("RakPeer::Receive got packet id %i len: %i", packet->data[0], packet->length);

		m_LastPacketID = (packet->length >= (sizeof(unsigned char) + sizeof(RakNetTime)) && packet->data[0] == ID_TIMESTAMP ?
			packet->data[sizeof(unsigned char) + sizeof(RakNetTime)] : packet->data[0]);
...
bool RakPeer::RPC( int uniqueID, const char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp, NetworkID networkID, RakNet::BitStream *replyFromTarget )
{
	...
	DbgOut("Trying to send RPC id %i", uniqueID);
...
//	char *writeTarget; // Used to hold either a block of allocated data or the externally allocated data
	...
	for (sendListIndex=0; sendListIndex < (unsigned)sendListSize; sendListIndex++)
	{
		...
		DbgOut("Transporting rpc id %i successfully", uniqueID);
...
{
	DbgOut("RakPeer::AddToBanList %s %u", IP, milliseconds);
...
int RakPeer::GetLastRPCID(void)
{
	return m_LastRPCID;
}

int RakPeer::GetLastPacketID(void)
{
...
#ifdef _DEBUG_LOGGER
	Log("[RPC] uniqueIdentifier: %d len: %i rpcIndex: %i", uniqueIdentifier, length, rpcIndex);
#endif
...
{
	DbgOut("RakPeer::SendImmediate ID: %i bits: %i priority: %i reliabil: %i", (BYTE)*data, numberOfBitsToSend, priority, reliability);
...
#endif
{
	...
	DbgOut("Packetid %i received", (unsigned char)(data)[0]);
...
//		char data[MAX_OFFLINE_DATA_LENGTH];
	...
	else if ((unsigned char)(data)[0] == (unsigned char) ID_CONNECTION_BANNED && length <= sizeof(unsigned char)*2)
	{
	...
	else if ((unsigned char)(data)[0] == (unsigned char) ID_NO_FREE_INCOMING_CONNECTIONS && length <= sizeof(unsigned char)*2)
	{
		// Remove the connection attempt from the buffered commands
		RakPeer::RequestedConnectionStruct *rcsFirst, *rcs;
		rcsFirst = rakPeer->requestedConnectionList.ReadLock();
	...
	else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_COOKIE && length == sizeof(unsigned char)*2+1) // 0.3.7 RAKSAMP HACK PONPON
	{
		...
		*(unsigned short*)&c[1] = (*(unsigned short*)&data[1] ^ NETCODE_CONNCOOKIELULZ);
	...
	if ( remoteSystem )
	{
		DbgOut("Datagram came from a connected player");
	...
	//	remoteSystemList[ remoteSystemIndex ].allowPlayerIdAssigment=true;
		if ( remoteSystemList[ remoteSystemIndex ].isActive )
		{
			...
			if (timeMS > remoteSystem->lastReliableSend && timeMS-remoteSystem->lastReliableSend > 5000 && remoteSystem->connectMode==RemoteSystemStruct::CONNECTED)
			{
				...
				{
					DbgOut("If no reliable packets are waiting for an ack, do a one byte reliable send so that disconnections are noticed");
			...
			{
				DbgOut("Reliability layer says we're cheaters");
				packet=AllocPacket(sizeof(char));
				packet->bitSize=8;
			...
			while ( bitSize > 0 )
			{
				DbgOut("ReliabilityLayer receive transported a packet to RakPeer id: %i", data[0]);
