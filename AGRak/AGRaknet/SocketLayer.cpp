...
int SocketLayer::RecvFrom( const SOCKET s, RakPeer *rakPeer, int *errorCode )
{
	...
	if (len > 0) DbgOut("recvfrom ret: s: %i l: %i l2: %i p: %i", s, len, len2, ntohs(sa.sin_port));
...
{
	DbgOut("SendTo s: %i l: %i addr: %u port: %u (ID? %i)", s, length, binaryAddress, port, (BYTE)*data);
	if ( s == INVALID_SOCKET )
	{
		return -1;
	}
