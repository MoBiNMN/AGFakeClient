...
//	bool duplicatePacket;
	...
	{
		DbgOut("Incoming packet has encryptor key");
...
//				internalPacketPool.ReleasePointer( internalPacket );
	...
	while ( internalPacket )
	{
...

		{
			...
			{
				char temp[256];
				sprintf(temp, "%p: Got packet %i data: %i bitlen: %i reliability: %d histogram: %X time: %lld",
					this,
			...
			{
				DbgOut("This packet is reliable, we will send acks for this one!, reliabil: %i", internalPacket->reliability);
			...
			{
				DbgOut("Underflow - got a packet we have already counted past");
			...
			{
				DbgOut("Got a higher count out of order packet whose messageNumber is higher than we have ever got");
			...
			if ( internalPacket->reliability == RELIABLE_SEQUENCED || internalPacket->reliability == UNRELIABLE_SEQUENCED )
			{
				...
				{
					DbgOut("Older sequenced packet. Discard it");
			...
			if ( internalPacket->splitPacketCount > 0 )
			{
				...
				DbgOut("An unsequenced split packet.  May be ordered though.");
...
//		printf("Using Pre-Allocated %i\n", internalPacket->data);
	...
	{
		DbgOut("ReliabilityLayer::Send it's split packet!");
...
void ReliabilityLayer::SendBitStream( SOCKET s, PlayerID playerId, RakNet::BitStream *bitStream )
{
	...
	if ( encryptor.IsKeySet() )
	{
		DbgOut("ReliabilityLayer::SendBitStream has encryptor key set");
	...
	*reliableDataSent = false;
	...
	if (time > nextAckTime)
	{
		if (acknowlegements.Size()>0)
		{
			output->Write(true);
			messagesSent++;
			DbgOut("Serializing acks to send...");
	...
	if (resendQueue.Size() > 0) DbgOut("ReliabilityLayer::GenerateDatagram resend queue more than 0 size: %u", resendQueue.Size());
...
int ReliabilityLayer::WriteToBitStreamFromInternalPacket( RakNet::BitStream *bitStream, const InternalPacket *const internalPacket )
{
	...
	if ( isSplitPacket )
	{
		DbgOut("WriteToBitStream is serializing into split packet!");
		bitStream->Write( internalPacket->splitPacketId );
		bitStream->WriteCompressed( internalPacket->splitPacketIndex );
		bitStream->WriteCompressed( internalPacket->splitPacketCount );
	}
